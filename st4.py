...
palabra_ingresada=st.text_input(label="Escribe el verbo conjugado")
...
def generador_conjugaciones(palabra):
    ...
    for i in [1,2,3]:
        ...
        for j in ["singular","dual","plural"]:
            ...
            valor_ratio=ratio(valor,palabra_ingresada)
...
if st.button('Conjugar'):
    resultado=generador_conjugaciones(palabra)
    diccionario=resultado[0]
    valor_max=resultado[1]
    persona=resultado[2]
    numero=resultado[3]
    #st.write(diccionario)
    st.write(persona)
    st.write(numero)
    st.write(diccionario[persona][numero])
    #st.write(valor_max)
    st.write(conjugar_verbo(palabra,persona,numero))
